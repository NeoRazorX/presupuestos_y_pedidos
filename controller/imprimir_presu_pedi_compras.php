 ...
 */
class imprimir_presu_pedi_compras extends fs_controller
{
   public $proveedor;
   public $impuesto;
   public $pedido;
   ...
   protected function process()
   {
      ...
      {
         $pres = new presupuesto_proveedor();
         $this->presupuesto = $pres->get($_REQUEST['id']);
         if($this->presupuesto)
         {
            $proveedor = new proveedor();
            $this->proveedor = $proveedor->get($this->presupuesto->codproveedor);
         }
   ...
   private function share_extensions()
   {
      $extensiones = array(
          ...
          array(
              'name' => 'imprimir_presupuesto_proveedor',
              'page_from' => __CLASS__,
              'page_to' => 'compras_presupuesto',
              'type' => 'pdf',
              'text' => ucfirst(FS_PRESUPUESTO).' simple',
              'params' => '&presupuesto=TRUE'
          ),
          array(
              'name' => 'email_presupuesto_proveedor',
              'page_from' => __CLASS__,
              'page_to' => 'compras_presupuesto',
              'type' => 'email',
              'text' => ucfirst(FS_PRESUPUESTO).' simple',
   ...
   private function generar_pdf_presupuesto($archivo = FALSE)
   {
      ...
      $pdf_doc->pdf->addInfo('Subject', ucfirst(FS_PRESUPUESTO).' de proveedor ' . $this->presupuesto->codigo);
         ...
         while( $linea_actual < count($lineas) )
         {
            ...
            $pdf_doc->add_table_row(
               array(
                   ...
                   'dato1' => $this->fix_html($this->presupuesto->nombre),
