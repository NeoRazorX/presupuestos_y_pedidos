 ...
 * Orientado para guardar datos adicionales para el desglose de las líneas del pedido.
 *
 * @author Carlos García Gómez <neorazorx@gmail.com>
 */
class pedidoprov_loc_linea_propiedad extends \fs_model
{
    /**
     * Nombre
     * @var string
    ...
    {
        parent::__construct('pedidoprov_loc_linea_propiedades');
        if ($a) {
            $this->name = $a['name'];
    ...
    {
        if (is_null($this->name) OR is_null($this->idpedido) OR is_null($this->idlinea)) {
            return FALSE;
        }
    ...
    public function save()
    {
        if ($this->exists()) {
            ...
            $sql = "INSERT INTO pedido_loc_linea_propiedades (name, idpedido, idlinea, text) VALUES ("
    ...
    public function array_save($idpedido, $idlinea, $values)
    {
        ...
        foreach ($values as $key => $value) {
            $aux = new \pedido_loc_linea_propiedad();
            $aux->name = $key;
